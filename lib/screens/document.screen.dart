...
import 'package:docs_clone_flutter/repository/socket.repository.dart';
  ...
  void fetchDocumentData() async {
    ...
    if (error!.data != null) {
      ...
      _controller = quill.QuillController(
          document: error!.data.content.isEmpty
              ? quill.Document()
              : quill.Document.fromDelta(
                  quill.Delta.fromJson(error!.data.content)),
          selection: const TextSelection.collapsed(offset: 0));
      setState(() {}); // Force rerender
    }

    _controller?.document.changes.listen((event) {
      // delta: entire new controller document
      // delta2: changes that were made
      // source
                  ...
                  child: ElevatedButton.icon(
                      onPressed: () {
                        Clipboard.setData(ClipboardData(
                                text:
                                    'http://localhost:3001/#/document/${widget.id}'))
                            .then((value) {
